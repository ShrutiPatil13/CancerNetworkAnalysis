...
#Clean and read sequences from fasta file
def sanitize(string):
...
def makeNetwork(filename,thresh,bthresh,type="", species=None,BL=""):
    ...
    if type=="PB":
        ...
        print(filename)
        start = time.time()
        C = createMinMatrix(species, True)
        C = np.asarray(C)
        end = time.time()
...
df_gene_seq_comb1_wildtype_uni = pd.read_csv('df_gene_seq_comb1_wildtype_uni_659.csv',header=0)
...
uni_seq = df_gene_seq_comb1_wildtype_uni.iloc[:,1]
uni_seq = uni_seq.values.tolist()
uni_labels = df_gene_seq_comb1_wildtype_uni.iloc[:,0]
uni_labels = uni_labels.values.tolist()

time_seq = df_gene_seq_comb1_wildtype_uni.iloc[:,[1]]
time_seq = time_seq.values.tolist()
df = pd.DataFrame()
df.insert(0, 'label', (range(len(uni_seq))))
df = df.applymap(str)
x_labels = df.iloc[:,[0]]
...
#diwann 
diwann_usa_Alltime=Graph()				
diwann_usa_Alltime=makeNetwork(rfile,thresh="",bthresh="",type="PB",species=time_seq)
plot(diwann_usa_Alltime,'diwann_293g659d.png',**style)
